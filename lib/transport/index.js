...
function Transport(context) {
  ...
  this.target = Object.freeze(extend({}, this._context.remote)); // userland
...
Transport.prototype._exec = function() {
  throw new Error('This transport does\'t implement `_exec(cmd, options)`');
};
...
Transport.prototype.transfer = function() {
  throw new Error('This transport does\'t implement `transfer(files, remoteDir, options)`');
};
...
Transport.prototype.prompt = function(message, options) {
  ...
  prompt.delimiter = '';
  prompt.message = chalk.gray('prompt');
  prompt.start();
  prompt.get({
    name: 'input',
    description: ' ' + chalk.white(message),
