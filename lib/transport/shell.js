...
Shell.prototype._exec = function(command, options) {
  ...
  byline(proc.stdout, { keepEmptyLines: true }).on('data', function(data) {
    if(!options.silent) {
      self._logger.stdout(String(data).trim());
    }
  });

  byline(proc.stderr, { keepEmptyLines: true }).on('data', function(data) {
    if(options.failsafe) {
...
Shell.prototype.transfer = function(files, remoteDir, options) {
  ...
  var tmpFile = tempWrite.sync(files);
