...
Transport.prototype = {
	...
	exec: function(args, opts) {
		...
		cmd = args.shift();
		opts = this._parseOptions(opts);
		return this.__exec(this.prefix + ' && ' + cmd, args.join(' '), opts);
	},
   ...
   * ```
   *
	 * @method prefixCommand(prefix, fn)
	 */

  prefixCommand: function(prefix, fn) {
  ...
  /**
   * Shorthand for calling prefixCommand('cd mydir', fn);
   *
	 * ```javascript
